...
async fn update_lockscreen_image(pic_filename : String) -> Result<(), Box<dyn std::error::Error>> {
    let file = StorageFile::GetFileFromPathAsync(pic_filename)?.await?;
    LockScreen::SetImageFileAsync(file)?.await?;
    Ok(())
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    copy(&mut data, &mut f)?;
    futures::executor::block_on(update_lockscreen_image(pic_file_path.clone()))?;
